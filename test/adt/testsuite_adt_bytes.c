...
static void test_adt_bytes_new_cstr(CuTest *tc) {
  CuAssertPtrEquals(tc, NULL, adt_bytes_new_cstr(NULL));

  const char *str = "Testing bytes";
  adt_bytes_t *bytes = adt_bytes_new_cstr(str);
  CuAssertPtrNotNull(tc, bytes);
...
static void test_adt_bytes_bytearray(CuTest *tc) {
  ...
  adt_bytes_t *bytes = adt_bytes_new(&data[0], sizeof(data));
  CuAssertPtrNotNull(tc, bytes);
  adt_bytearray_t *array = adt_bytes_bytearray(bytes);
...
static void test_adt_bytes_vdelete(CuTest *tc) {
  adt_bytes_t *bytes = adt_bytes_new_cstr("cleanup");
  CuAssertUIntEquals(tc, 7, adt_bytes_length(bytes));
  adt_bytes_vdelete((void *)bytes);
}
